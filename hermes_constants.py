...
def get_default_hermes_root() -> Path:
    """Return the Hermes root directory used for profile-level operations.

    Rules:
    - If HERMES_HOME is unset, use ``~/.hermes``.
    - If HERMES_HOME points inside ``~/.hermes`` (including profile paths like
      ``~/.hermes/profiles/coder``), the root is still ``~/.hermes``.
    - If HERMES_HOME points outside ``~/.hermes`` (custom/docker/disposable
      deployment), treat that custom path as the root unless it is a profile
      path ``<root>/profiles/<name>``, in which case return ``<root>``.
    """
